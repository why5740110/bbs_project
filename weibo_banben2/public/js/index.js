...
// $(function () {
	...
	// $('#picture').click(function(){
		...
		// width : 120,	//上传按钮宽度
		// height : 30,	//上传按钮高度
		// $(this).src("background",) + '/Uploadify/browse-btn.png',	//上传按钮背景图地址
	...
	$('.send_write textarea').focus(function () {
		...
		$(this).css('borderColor', '#FFB941').keyup(function () {
			...
			if (lengths[0] > 0) {//当前有输入内容时改变发布按钮背景
				$('.send_btn').css('backgroundPosition', '-218px -50px');
				// 在里面绑定鼠标移入事件
				$('.send_btn').mouseover(function(){
					$('.send_btn').css('backgroundPosition', '-134px -50px');
				}).mouseout(function(){
					$('.send_btn').css('backgroundPosition', '-218px -50px');
				});
			} else {//内容为空时发布按钮背景归位
