...
$(function () {
	...
	$(".mini_img").live("click", function(){
		var mini = $(this).attr("src");
		$(this).parent().find(".img_tool").show();
		$(this).parent().find(".img_info").children("img").attr("src", mini);
		$(this).parent().find("a").attr("href", mini);
	});
	 ...
	 */
	 ...
	 $(".comment_btn").live("click", function(){
	 	var neirong = $(this).parent().prev();
		var content = $(this).parent().prev().val();
	 	var url = $(this).attr("url");
	 	var article_id = $(this).attr("wid");
	 	var dangqian = $(this).parent().next();
	 	var pinlunshu = $(this).parent().parent().prev().prev().find(".pls");
	 	var shuzi = parseInt($(this).parent().parent().prev().prev().find(".pls").html());
		$.ajax({
	 		url:url,
	 		type:'post',
	 		...
	 		success:function(data){
	 			if ( data ) {
	 				dangqian.prepend("<li class=\"pinlun\" style=\"margin-bottom:10px;\"><span style=\"font-size:14px;color:#888888;\">"+data+":</span>"+content+"</li>");   
		 			showTips("评论成功！", 1, 780);
		 			neirong.val("");
		 			shuzi = shuzi+1;
		 			pinlunshu.html(shuzi);			
	 			}
	 		},
	 	});
	 ...
	 */
	 $(".keep").live("click", function(){
	 	...
	 	var shoucang = $(this).find("span");
	 	var num_a = $(this).find("a");
	 	var num = parseInt(num_a.html());
	 	$.ajax({
	 		url:url,
	 		type:'post',
	 ...
	 */
	 $(".dianzan").live("click", function(){
	 	...
	 	var zantu = $(this).find("img");
	 	var num = parseInt($(this).next("span").html());
	 	// alert(num);
	 	$.ajax({
	 		url:url,
	 		type:'post',
	 		...
	 		success:function(data){
	 			if (data == "取消点赞成功！") {
	 				if (num==0) {
	 					nextspan.text(num);
	 					zantu.attr("src", "/weibo/public/Images/weizan.png");
	 				} else {
	 					num = num - 1;
		 				nextspan.text(num);
		 				zantu.attr("src", "/weibo/public/Images/weizan.png");	
	 			...
	 			if (data == "点赞成功！") {
	 				num = num + 1;
	 				nextspan.text(num);
	 				zantu.attr("src", "/weibo/public/Images/yizan.png");
	 			}
	 			showTips(data, 1, 780);
	 		},
	 	});
   ...
   // 分组移动特效
   $(".fenzu").mouseover(function(){
       $(".user_fen_group").hide();
       var id = $(this).attr("data");
       $(this).parent().next().show();
   });

   $("#create_group").mouseover(function(){
       $(".user_fen_group").hide();
   });
   ...
   $('.add-follow-sub').click(function () {
   		...
   		$.ajax({
   			url:url,
            type:"post",
            data:{uid:follow,groupname:group}, //参数
   ...
   $('.add-fl').live("click",function(){
   		var haoyouid = $(this).attr('uid');
   		var url = $(this).attr('data');
   		var id = $(this).attr('userid');
   		$.ajax({
            url:url,
            type:"post",
            data:{id:id,haoyouid:haoyouid}, //参数
            dataType:"json", //返回的数据类型
            async:true, //是否异步
            success:function(data){
            	// 获取数据库传过来的当前用户下面的组信息
            	...
            	if (data) {
	            	for ( var i = 0 ; i < data.length; i++ ) {
	            		$("#gid").append("<option class='yiguanzhu' data="+data[i]['id']+">"+data[i]['groupname']+"</option>");
	            		// 获取左边所有分组
	            	}	
            	}
            	//alert(data.length);
...
function showTips(tips,time,height){
	var top = $(document).scrollTop() + ($(window).height() - $('.tipsClass').height()) / 2;
	top = top - 200;
	var windowWidth = $(window).width();top=top?top:$(window).height();
	time = time ? time : 1;
	var tipsDiv = '<div class="tipsClass">' + tips + '</div>';
	var left = ($(window).width() - $('.tipsClass').width()) / 2;
	$( 'body' ).append( tipsDiv );
	$( 'div.tipsClass' ).css({
		'top' : top + 'px',
