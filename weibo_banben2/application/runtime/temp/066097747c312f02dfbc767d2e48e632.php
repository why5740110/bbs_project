...
<!--/二级导航-->        <!--/二级导航-->
	 ...
	 <div class="F_rows_main"> 
      <div class="F_cols_main">
		...
		<br>
            <div class="phto_count M_txta" node-type="photos_info">我的照片<span>(共<?php echo $pic_count; ?>张)</span></div><div class="browse_type"><!-- <a class="M_linke M_slide_sm" href="http://photo.weibo.com/2664098951/photos#" action-type="slide_open"><span class="M_ico M_icon12"></span>幻灯浏览</a> --></div>
        </div>
        <!--/幻灯浏览-->
        ...
        <div class="m_photo_list_a" node-type="photo_list">
			...
			<ul class="photoList clearfix">
				<?php foreach($photopiclist as $vt): ?>
				<li action-data="pid=3642504728738567&amp;aid=3423857315111989&amp;type=2">
					<dl class="m_photoItem m_photoItem_a">
						<dt class="photo myphoto" style="border:1px solid #ccc;">
							<a href="<?php echo U('Pic/picshow'); ?>?picid=<?php echo $vt['id']; ?>&uid=<?php echo $vt['uid']; ?>&photoid=<?php echo $vt['photo_id']; ?>" title="">
								<img src="<?php echo APP_INDEX; ?>uploads/pic/<?php echo $vt['picname']; ?>" width="200">
							</a>
						</dt>
						<dd>
							<div class="mark_box M_txtb">
								<p class="time"><?php echo date("Y-m-d",$vt['addtime']); ?></p>
							...
							<p class="picid" style="display:none;"><?php echo $vt['id']; ?></p>
							<p class="puid" style="display:none;"><?php echo $vt['uid']; ?></p>
							<p class="photoid" style="display:none;"><?php echo $vt['photo_id']; ?></p>
							<p class="desc" title=""></p>图片描述:
							<span class="description"><?php echo $vt['descrip']; ?></span>
							<div class="edit_tip">
								...
								<span class="M_vline M_txtd"></span>
								<a href="javascript:void(0);" action-type="photo_fmian">设为封面</a>
							</div>
						</dd>
					</dl>
				</li>
				...
				<script>	
					$(function(){
						//选择删除按钮 并绑定点击事件 使用ajax提交到后台处理页面
							...
							if(confirm("确定删除吗?")){
								$.ajax({
									...
									success:function(){
										...
										window.location.href="<?php echo U('Pic/photopicshow'); ?>?uid="+puid+"&photoid="+photoid;
										alert('删除成功');
									},
									error:function(){
										alert('删除失败！');
						...
						//选择设为封面按钮 并绑定点击事件 使用ajax提交到后台
						$("a[action-type='photo_fmian']").click(function(){
							//获取图片id
							picid = $(this).parent().parent().children(".picid").html();
							//获取用户uid
							puid = $(this).parent().parent().children(".puid").html();
							...
							if(confirm("是否设置封面?")){
								$.ajax({
									url:"<?php echo U('Pic/updatefm'); ?>",
									type:'post',
									data:{picid:picid,photoid:photoid},
									dataType:'html',
									async:true,
									success:function(){
			...
			$("a[action-type='save']").click(function(){
				...
				$.ajax({
					...
					success:function(){
						...
						window.location.href="<?php echo U('Pic/index'); ?>";
						alert("编辑成功！");
					},
					error:function(){
						alert("编辑失败！");
	 ...
	 <div class="L_layer" node-type="outer" style="position: absolute; z-index: 100100; visibility: visible; top: 291px; left: 413px;display:none;" id="layer_14569860537111" stk-mask-key="14569860537112">
      ...
      <div class="F_cols_second">
		<br>
      	<div class="album_count M_txta" node-type="album_info">共<?php echo $photo_count; ?>个专辑</div>
      	<!-- 相册专辑列表 -->
        <div class="m_piclist">
          <ul node-type="album_list">
			<?php foreach($photo_list as $pl): ?>
			<li action-data="aid=3423857315111989" style="border:1px solid #ccc;">
				<div class="m_cover m_cover_a" >
					<div class="img_wrap" style="border:1px solid #ccc;">
						<a title="<?php echo $pl['title']; ?>" href="<?php echo U('Photo/index'); ?>?uid=<?php echo $pl['uid']; ?>">
							<img height="90" width="90" src="<?php echo APP_INDEX; ?>uploads/pic/<?php echo $pl['photoname']; ?>">
						</a>
					</div>
				</div>
				<p style="color:red;font-weight:bold;">
					<a title="<?php echo $pl['title']; ?>" href="<?php echo U('Photo/index'); ?>?uid=<?php echo $pl['uid']; ?>" class="M_linkb"><?php echo $pl['title']; ?></a>
				</p>
				<p class="M_txtb"><?php echo $pl['content']; ?></p>
			</li>
			<?php endforeach; ?>
			<p><a href="<?php echo U('photo/index'); ?>">查看更多...</a></p>
		  </ul>
